...
class Device
{
	...
	* @param {string} providerType timeline's provider type "Device" "SyncController" 
	 */
	addTimeline(timelineId, sessionId, contentId, timelineType, channel, providerId, providerType)
	{
		var priv = PRIVATE.get(this);
		var self = this;
		...
		return new Promise((resolve, reject) =>{
			...
			self.reload().then(()=>{
				...
				}else
				{
					...
					var t = new Timeline(timelineId, sessionId, contentId, timelineType, channel, self.id, providerType);
				...
				// console.log("Device.AddTimeline() result: ", result);
				resolve(true);
			}).catch(error=>{reject(error);});			
