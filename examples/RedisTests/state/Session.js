...
class Session
{
		...
		return new Promise((resolve, reject) =>{
			...
			ds.getAsync(key).then((result)=>{
				...
				if (typeof result!=="undefined" && result !==null)
				{
					...
					s.syncControllerId = obj.syncControllerId;
					resolve(s);	
				}
				else
					resolve(null);
	...
	getTimelines(queryOptions)
	{
		...
		return new Promise((resolve, reject) =>{
			...
			self.reload().then(()=>{
				...
				{
					console.log("Session.getTimelines() : get timeline for device: ", dev.id );
	...
	removeDeviceFromContext(deviceId, contextId)	
	{
		...
		return new Promise((resolve, reject) =>{
			self.reload().then(()=>{
				...
				if (context)
				{
					console.log("Removed device from context '" , context.id, "'");
	...
	/**
	 * Register a new timeline in this session. It assumes tha
	 ...
	 * @returns {Promise}
	 */
	addTimelineToDevice(timelineId, contentId, timelineType, channel, providerId, contextId)
	{
		var self = this;
		
		return new Promise((resolve, reject) =>{
			// console.log("Session.addTimelineToDevice() : 1");
			self.findDevice(providerId, contextId)
				.catch((error)=>{ reject(error);})
				.then((device)=>{	
					// console.log("Session.addTimelineToDevice() : 2");				
					if(device)
					{	
						// console.log("Session.addTimelineToDevice() : 3");
						// console.log("Session.addTimelineToDevice() : ", device.serialise());
						return device.addTimeline(timelineId, self.id, contentId, timelineType, channel, providerId, "Device");
					}else
					{
						return Promise.resolve(false);
					}
				}).then((result)=>{
					// console.log("Session.addTimelineToDevice() result: ", result);
					resolve(result);
				}).catch((error)=>{ reject(error);});
		});
	...
	serialise()
	{
		...
		if (typeof this.syncControllerId !== "undefined")
			obj.syncControllerId = this.syncControllerId;
		
		if (this.syncTimeline instanceof Timeline)
		{
			obj.syncTimeline = this.syncTimeline.id;
		}else if (typeof this.syncTimeline === "string")
		{
			obj.syncTimeline = this.syncTimeline;
		}
			...
			self.reload().then(()=>{
				...
				if (contexts.length > 0)
				{
					...
					for (let c of contexts)
					{
						if (c instanceof Context)
						{
							console.log("context '", c.id, "' deleted");
							var q = c.cleanUp(ds);
							promises.push(q);
						}
					}
